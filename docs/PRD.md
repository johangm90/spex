...
The problem it solves: when multiple LLM-based agents collaborate on a software project, there is no authoritative shared state — each session starts blind, agents duplicate work, and the human developer loses traceability. Spex provides that shared brain: a single Rust binary that runs a local MCP server, persists project state in SQLite, bundles 16 agent definition files, and enforces a human-gated spec lifecycle.
...
3. **One binary, works anywhere** — `spex` is a self-contained Rust binary: it embeds 16 bundled agent markdown files, bundles the MCP server, includes migrations, and installs via a single shell command. No Node.js, no Python, no Docker required.
...
| **Asset embedding** | `include_dir` | 16 bundled agent markdown files from `agents/` compiled into the binary |
...
### MCP State Server
The agent-facing API. JSON-RPC 2.0 over stdio. 49 canonical tools covering specs, tasks, events, memory, artifacts, project-context bootstrap, policy, evidence, readiness, sessions, evals, and PRD reading/writing. Started via `spex mcp serve`.

### Skills & Agent Bundle
16 bundled agent markdown files embedded from `agents/` at compile time. Installed to `~/.config/opencode/agents/` via `spex setup` or `spex skill install --all`. Separately, custom project skills are generated as `SKILL.md` files under `~/.agents/skills/<slug>/`.
...
6. ~~**Memory system evolution** — How deeply should the memory layer evolve toward Engram-style features (FTS5, typed observations, topic keys)? Is this a minor enhancement or a separate slice?~~ **Resolved:** Memory Evolution pass shipped: `memory_list` with filtering/pagination, `memory_gc` with FTS rebuild, `spex memory` CLI, QueryBuilder refactor. Current MCP surface: 38 canonical tools.

7. ~~**MCP tool alias cleanup** — IMP-008: 27 tool registrations for 14 operations. Should aliases be hidden behind a `--legacy` flag, or kept first-class forever for backward compatibility?~~ **Resolved:** Current `build_tools_list()` exposes 38 canonical tools and the earlier "27 registrations" claim is stale.
