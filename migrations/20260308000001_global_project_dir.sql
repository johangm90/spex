...
--
-- IMPORTANT - ordering rationale:
--   SQLx wraps each migration in a transaction.  Inside a transaction,
--   PRAGMA foreign_keys = OFF is a no-op in SQLite (the FK pragma cannot be
--   changed within a transaction).  Even though FK *enforcement* is usually
--   disabled by default, SQLite still *validates* FK declarations structurally
--   at statement-compile time.  After specs(id) loses its standalone PK status,
--   any table whose CREATE still says REFERENCES specs(id) causes:
--     "foreign key mismatch - \"<table>\" referencing \"specs\""
--   when SQLite compiles the INSERT…SELECT against the old table.
--
--   Fix: rebuild all FK-child tables FIRST (before specs/tasks are touched).
--   Once those tables are rebuilt without FK declarations the parent tables
--   (specs, tasks) can be safely restructured.
--
-- Rebuild order:
--   Phase A (FK children — rebuilt first, no FKs in new schema):
--     incidents, context_gaps, verification_runs, interrupts,
--     handoff_snapshots, plan_versions, task_leases, task_locks,
--     replan_requests
--   Phase B (FK parents — rebuilt after children are clean):
--     specs, tasks
--   Phase C (no-FK tables — order doesn't matter):
--     events, memory
--
...
-- ─────────────────────────────────────────────────────────────────────────────
-- PHASE A  — FK-child tables rebuilt first (no FK clauses in new schema)
-- ─────────────────────────────────────────────────────────────────────────────

-- ── A-1. incidents ───────────────────────────────────────────────────────────
--    OLD PK:  id TEXT PRIMARY KEY, FK → specs(id), tasks(id)
--    NEW:     rowid PK, UNIQUE(project_dir, id), no FK clauses
...
-- ── A-4. interrupts ──────────────────────────────────────────────────────────
--    OLD PK:  id TEXT PRIMARY KEY, FK → specs(id)
--    NEW:     rowid PK, UNIQUE(project_dir, id), no FK clauses
...
-- ── A-5. handoff_snapshots ───────────────────────────────────────────────────
--    OLD PK:  id TEXT PRIMARY KEY, FK → specs(id), interrupts(id)
--    NEW:     rowid PK, UNIQUE(project_dir, id), no FK clauses
--    Note: interrupts was already rebuilt above → no structural mismatch.
...
-- ── A-6. plan_versions ───────────────────────────────────────────────────────
--    OLD PK:  id TEXT PRIMARY KEY, UNIQUE(spec_id, version), FK → specs(id)
...
-- ── A-7. task_leases ─────────────────────────────────────────────────────────
--    OLD PK:  task_id TEXT PRIMARY KEY REFERENCES tasks(id)
--    NEW:     PRIMARY KEY (project_dir, task_id), no FK clauses
...
-- ── A-8. task_locks ──────────────────────────────────────────────────────────
--    OLD PK:  id TEXT PRIMARY KEY, FK → tasks(id), specs(id)
--    NEW:     rowid PK, UNIQUE(project_dir, id), no FK clauses
...
-- ─────────────────────────────────────────────────────────────────────────────
-- PHASE B  — FK-parent tables (specs, tasks) rebuilt after children are clean
-- ─────────────────────────────────────────────────────────────────────────────
