...
pub async fn create_handoff_snapshot(
    ...
    sqlx::query(
        "INSERT INTO handoff_snapshots (id, spec_id, interrupt_id, last_wave, last_task, files_touched, decisions, open_risks, next_steps, created_at)          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    )
    .bind(id)
    .bind(spec_id)
    .bind(interrupt_id)
    ...
    get_handoff_snapshot(pool, id)
        .await?
        .ok_or_else(|| anyhow!("Failed to create handoff snapshot '{}'", id))
}

pub async fn get_handoff_snapshot(pool: &SqlitePool, id: &str) -> Result<Option<HandoffSnapshot>> {
    let row = sqlx::query_as::<_, (String, String, Option<String>, Option<i64>, Option<String>, String, String, String, String, String)>(
        "SELECT id, spec_id, interrupt_id, last_wave, last_task, files_touched, decisions, open_risks, next_steps, created_at FROM handoff_snapshots WHERE id = ?"
    )
    .bind(id)
    .fetch_optional(pool)
    .await?;
...
pub async fn list_handoff_snapshots(
    ...
    let rows = if let Some(spec) = spec_filter {
        sqlx::query_as::<_, (String, String, Option<String>, Option<i64>, Option<String>, String, String, String, String, String)>(
            "SELECT id, spec_id, interrupt_id, last_wave, last_task, files_touched, decisions, open_risks, next_steps, created_at FROM handoff_snapshots WHERE spec_id = ? ORDER BY created_at DESC, id"
        )
        .bind(spec)
        .fetch_all(pool)
        .await?
        ...
        sqlx::query_as::<_, (String, String, Option<String>, Option<i64>, Option<String>, String, String, String, String, String)>(
            "SELECT id, spec_id, interrupt_id, last_wave, last_task, files_touched, decisions, open_risks, next_steps, created_at FROM handoff_snapshots ORDER BY created_at DESC, id"
        )
        .fetch_all(pool)
        .await?
    };
